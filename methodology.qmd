...
A multitude of remarks can be drawn from this definition: First, the expected distribution of weather realizations $\psi ( \mathbf{C}_{i \tau} )$ can never be parametrized by its empirical counterpart $\psi ( \mathbf{c}_{i \tau} )$, that characterizes the distribution observed weather states $\mathbf{v}_{i, t \in \tau}$.
...
-   paragraph explaining how this links to the ATE identification introduced by Holland (1986); **see also in Bachelor's thesis**
...
-   Also, unit homogeneity assumption does not hold, which makes OVB problem even harder to avoid
