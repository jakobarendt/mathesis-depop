...
-   Panel model: Frequency-identification-trade-off - longer time period better for inferring climate and climate change vs. control and treatment population are better comparable for shorter time periods (identification more reliable for shorter periods)
...
Currently foreseen control variables and FEs:

-   LAU-FEs
-   **remember that Country-FEs are not needed as they are already captured by LAU-FEs**
-   Time-FEs or, alternatively,
-   Country-Time-FEs, i.e. two-way FE model (generally captures more, i.e. better control in theory, but try Country-Time-trend if the Country-Time-FEs capture too much)
...
$$
\begin{aligned}
\mathrm{pop}_{ict} &= \beta_{\mathrm{TS}} \, \mathrm{temp}_{ict} + \beta_{\mathrm{P}} \, g ( \mathrm{prec}_{ict} ) + \omega_i + \rho_c + \epsilon_{ict}\\
\mathrm{pop}_{ict} &= \beta_{\mathrm{TS}} \, \mathrm{temp}_{ict} + \beta_{\mathrm{TDR}} \, ( \mathrm{temp}_{ict} \cdot \pi_{i} ) + \beta_{\mathrm{P}} \, g ( \mathrm{prec}_{ict} ) + \omega_i + \rho_c + \epsilon_{ict}
\end{aligned}
$$
...
\[**Not sure about whether this interaction term is correct**\]

Similarly, a more extensive version the above model, including trend effects is represented by the equation

$$
\begin{aligned}
\mathrm{pop}_{ict} &= \beta_{\mathrm{TS}} \, \mathrm{temp}_{ict} + \beta_{\mathrm{TDR}} \, ( \mathrm{temp}_{ict} \cdot \pi_{i} ) + \beta_{\mathrm{P}} g ( \mathrm{prec}_{ict} ) + \omega_i + \rho_c + \delta \cdot \text{t} + \epsilon_{ict}
\end{aligned}
$$

where $\delta \cdot \text{t}$ is the trend factor capturing all general trends present in all LAUs.
...
-   This can also simply be estimated with OLS to determine whether any changes caused by weather differences between the last and the first decade can be attributed to any population changes:
    ...
    -   $\omega_i$ is a dummy that controls for LAU-specific particularities
    -   similarly, $\rho$ is a country-specific dummy, and
    -   $\phi_{ic}$ is the regression error.

\[Uncertainties regarding this sub-chapter:\]

-   einzelner dummy für ländlich und nicht nicht notwendig --\> tatsächlich?
-   FEs noch: country-zeit oder zeit (ersteres wahrscheinlich besser) --\> dann aber noch trend sinnvoll? dann aber auf jeden Fall keine Country-FEs mehr benötigt?
...
\[Open to-do's for this sub-chapter\]
...
-   Reverse causality? More populated areas are likely to have higher surface temperatures, but this cancels out with the hypotheses that hotter LAUs have population reductions?
...
-   **New:** (On adaptation) In my case, adaptation would likely lead to an attenuation bias in the effect estimations. For the panel model, would it be right to say that adaptations that influence the climate-socioeconomic causal relationship in a constant way over time are account for due to the time series nature of the panel model?
    -   country-FE, LAU-FE catch differences in adaptation capability of LAUs
    -   trend-variable captures influence of adaptation on climate-socioecon relationship that is constant over time (even better: trend\*LAU); account for potential spurious correlations, where climate and population accidentally exhibit the same development (?? see again Marbler 2024 for this)
