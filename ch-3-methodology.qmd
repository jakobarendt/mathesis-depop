...
They discuss in more detail under which circumstances weather observations can reliably describe climate and changes therein, and the advantages and disadvantages of different econometric models aimed at identifying its socioeconomic impacts.
...
[^ch-3-methodology-1]: Or, to be more precise, the statistical moments that parametrise this distribution.
...
Unless when making assumptions about the shape of the distribution specifying climate, we are unlikely to ever know the full distribution and, hence, the climate perfectly as we only observe weather once at the given location during the specific time period, and do not know which other states of the atmosphere could have realised.
...
This definition has the advantage that it leaves room for adjustments in order to allow application to the data and the research question at hand: first, it allows for using partially or fully model-derived data such as reanalyses, besides observed weather data, to be used for deriving climate impacts.
...
Rather, their appropriate choice should be driven by the research question and the socioeconomic outcome investigated, e.g. growing degree days for agricultural outcomes and temeperature adjusted for evaporative cooling for public health outcomes [see also @kolstad-moore-2020-clim-impacts-from-weather-data, p. 3] **TODO add column 'Weather statistic' to table**.
In general tendency, the weather parameters to describe climate have increased in number with each new study applying climate econometric models [@hsiang-2016-climate-econometrics, p. 45].
...
The belief effect together with its interaction with the direct effect is often considered as *adaptation*. In the previous example, people that adapt to a higher likelihood of sunburns are those that expect high UV-exposure and put on sunscreen.
In this example, adaptation leads to an attenuation of the effect in the relationship "closeness to the equator" and "likelihood of sunburns".
For applied estimation with time series models based on short period distances, @hsiang-2016-climate-econometrics argues that estimated short-run effects should be the same as the long-run effects accounting for adaptation so long as the socioeconomic outcome variable can be assumed to result from continuous maximisation, whereas @merel-gammans-2021-climate-econometrics-lr-adaptation find these models to estimate a weighted average of short- and long-run responses and they propose a correction term for this bias **TODO check whether to better put this sentence at the summary or modelling paragraph**.
...
**TODO check whether this is correctly placed here** As @hsiang-2016-climate-econometrics and @kolstad-moore-2020-clim-impacts-from-weather-data explain, there are two fundamental issues in climate econometrics **\[CONTINUE HERE\]**.
Each of these issues have solutions that stand against each other in a trade-off that needs to be balanced.
First, climate needs to be reliably inferred from weather observations (clear that different regions have different climates at one point in time, but OVB problem).
Second, changes in climate need to be sufficiently relatable in terms of time and space to the outcome observed.
