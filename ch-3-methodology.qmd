...
## The Climate Econometric Problem and Common Approaches for Its Solution
...
They discuss in more detail under which circumstances weather observations can reliably describe climate and changes therein, and the advantages and disadvantages of different econometric models aimed at causally identifying its socioeconomic impacts.
...
In other words, climate is the parametrisation of the joint distribution of athmospheric states for a given location and specified time period, based on which certain potential states of weather could realise.
...
As long as a change in weather can reliably represent a change in climate, the parameters of the full distribution representing the climate do not need to be perfectly known.
...
Rather, their appropriate choice should be driven by the research question and the socioeconomic outcome investigated, e.g. growing degree days for agricultural outcomes or temperature adjusted for evaporative cooling for public health outcomes [see also @kolstad-moore-2020-clim-impacts-from-weather-data, p. 3] **TODO add column 'Weather statistic' to table**.
...
Returning to the latter example, individuals may expect their UV-exposure to be high and put on sunscreen.
It is only the combination of these two effects that constitute the effects of climate as they can also attenuate or reinforce each other, and during estimation of real-world effects they are also hard to differentiate.[^ch-3-methodology-2]
The belief effect together with its interaction with the direct effect is often considered as *adaptation*. Considering again the previous example, people that adapt to a higher likelihood of sunburns are those that expect high UV-exposure and put on sunscreen.
...
For applied estimation with time series models based on short period distances, @hsiang-2016-climate-econometrics argues that estimated short-run effects should on the margin be the same as long-run effects incorporating adaptation so long as the socioeconomic outcome variable can be assumed to result from continuous maximisation, whereas @merel-gammans-2021-climate-econometrics-lr-adaptation find these models to estimate a weighted average of short- and long-run responses and they propose a correction term for this bias **TODO check whether to better put this sentence at the summary or modelling paragraph**.

[^ch-3-methodology-2]: While direct effects can also be substantiated for very short time periods, belief effects and changes thereof are often traceable only over the longer run, with data sets covering longer time periods.

**TODO check whether this is correctly placed here** As @hsiang-2016-climate-econometrics and @kolstad-moore-2020-clim-impacts-from-weather-data explain, there are three fundamental challenges in climate econometrics, and the econometric methodologies most common in the literature are subject to a trade-off: they can mitigate either one of the challenges in isolation, but none are able to address them both simultaneously **\[CONTINUE HERE\]**.
The first challenge is to ensure that changes in the outcome variable can be associated...
This can be ensured by ..., and @hsiang-2016-climate-econometrics call this the *unit homogeneity assumption*.
The second challenge is to reliably infer climate from weather observations.
(clear that different regions have different climates at one point in time, but OVB problem).
This also relies on an assumption, which @hsiang-2016-climate-econometrics consider the *marginal treatment comparability assumption*.
Third and last, it is also a challenge to credibly identify changes in outcome to climate with regards to the time length and frequency if the data at hand requires time series based modelling.
Dual trade-off with regards to time...
...
To be able to surely associate any differences in units' outcomes to differences in climate, all of their other differentiating characteristics that influence the outcome have to be known and controlled for.[^ch-3-methodology-3]
...
This has the advantage that they are much less prone to confounding due to unobserved or non-measurable characteristics also influencing outcomes,[^ch-3-methodology-4] but it comes at the cost that climate and/or changes thereof are harder to determine.
...
[^ch-3-methodology-3]: This results from the *unit homogeneity assumption* further discussed in the context of the identification problem and the Rubin Causal Model in @sec-MethodologySubsecIdentificationProblem.

[^ch-3-methodology-4]: They rely on a weaker form of the *unit homogeneity assumption*, the *marginal treatment comparability assumption*.
    This is also explained more formally in @sec-MethodologySubsecIdentificationProblem.

spatial first differences for mitigating weaknesses in cross-section (Druckenmiller and Hsiang, 2018, @linsenmeier-2023-temp-econ-dev-sfd).

To summarise, for weather to serve as a reliable proxy for climate, a few things need to be considered.
First, ...
using weather observation for an **appropriate description** of the weather, e.g. mean The quality of the identification of climate effects depends mainly on how appropriate and relevant the chosen weather parameters are for the outcome variable, e.g. in agriculture they can reflect the specific growing characteristics or agricultural processes whose outcomes are observed, or in sociology and conflicts they can make up the weather parameters most relevant for human social behaviour.
...
\[TODO: paragraph about specific techniques in different papers\] also how recent developments have tried improve upon their weaknesses, i.e. spatial first differences for weaknesses in cross-section (Druckenmiller and Hsiang, 2018, @linsenmeier-2023-temp-econ-dev-sfd)

## Identification Problem and Methodology {#sec-MethodologySubsecIdentificationProblem}

\[Rubin Causal Model - Empirical problem when trying to identify causal effects - Reliance on natural experiments - Cite Holland (1986) here & look into Bachelor's thesis - *Unit homogeneity assumption*, i.e. absent any treatment and controlling for observable differences, treatment and control units should have the same expected outcomes, this is the case when when they exhibit no differences in unobservables\]
...
\[Name some papers (the same as mentioned just above or others) to make a differentiation by methodology, i.e. simple cross-section analysis, panel analysis, *spatial first differences* etc, maybe also just for table in conceptual framework chapter\]

-   Identifying variation as a key word
-   Unlike for certain agricultural processes or economic outcomes, we may never fully know the data-generating process and influencing factors for demographic developments. Considering that, panel modeling is very likely a better choice than a cross-section with its a lack of a systematic approach to test for missing variables.
-   OVB: panel models are less prone to omitted variable bias, that is, the existence of a variable that also impacts the outcome and is correlated with climate or one of the other observables used as controls.
...
-   endogeneity due to weather reporting unlikely
-   bad controls are to be avoided (i.e. separately specifying regression variables that are already part of the causal chain of climate or another control variable)
-   Note about panel models: require *marginal treatment comparability assumption* and the variables specifying climate need to be comprehensive to ensure its exogeneity when considering the climate's impact on a population (e.g. temperature may have a significant variation in one temperature, but another climate variable that exhibits significant variation in the following period as consequence of temperature may not be captured and this becomes a problem).
