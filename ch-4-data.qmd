...
The steps undertaken for their preparation are described more extensively in [appendix @sec-AppxPopData] and [appendix @sec-AppxWeatData], analogously.
...
The data covers LAUs of European Union (EU) member states, certain candidate and European Free Trade Association (EFTA) countries, as well as the United Kingdom, and can be downloaded from its website [@eurostat-2023-lau-website].
...
Additionally, census time points vary among countries and regions, which complicates cross-section (intra-period) comparability as this requires unified time points.
...
Frequency and scope of such reforms of administrative boundaries was diverse among countries.
For this reason, the techniques chosen to adjust the spatial attributes of the population figures to the same base year vary with the country and in two cases even with the region (Germany and the UK).
For example, for some countries a backward approach was applied that adjusted only the figures of those LAUs subject to known boundary changes.
For other countries however, spatial overlays of decade-specific boundary maps were performed to adjust all LAU figures of an entire or even multiple time periods.
...
In addition to the partially estimated figures to ensure their harmonisation as well as the original ones, the data is delivered with unique identifiers that can reference the figures to geospatial vector data, more specifically shapefiles.
This way, the historical population figures can be integrated into maps and other spatial analyses.
For the majority of the countries, these georeferences correspond to the *EuroBoundaryMaps* [@eurogeographics-2025-euroboundarymap-website; @eurostat-2025-gisco-copyright-website] versions of the years 2011 or 2012, as the population figures were harmonized to LAU boundaries of either of the two years.
Nonetheless, some countries' LAU population figures are delivered with references to different geospatial vector data, for which the shapefiles are provided.
