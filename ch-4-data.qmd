...
-   Lack of differentiation of migration from net fertility and mortality rates is on purpose (all three aspects are expected to be influenced by the climate, and the latter two population distribution drivers are also of interest as they might also differ with the different climates in the rural and urban geographies)
-   Make table with summary statistics (mean, SD, min, max) for each variable (climate change and period before panel - temperature, precipitation, moisture; n - sum of available time periods for all units if panel unbalanced; rural population share change and initial/before panel; land area)
...
-   Consider whether this is reanalysis or observational weather data
-   Discuss more broadly the long-term state and influence of the climate on the European continent (i.e. shaped by gulf stream, geography, topology and other factors)
...
Essentially, the question as to if and how weather data can be used to estimate impacts of climate and climate change boils down to aspects -- whether these realisations of their underlying (joint) probability distribution characterising the climate are sufficient to describe that distribution, and whether any changes in these weather realisations can be assumed to reflect well enough any changes in the underlying distribution characterising the climate.
...
-   Three drivers determining human population at any given location and point in time:
    -   Birth rate
    -   Death rate
    -   Migration
-   Factors relevant for all of these three drivers and on which climate has a proven impact:
    -   Agriculture
    -   Human health
    -   Energy demand
-   Quality of living aspect sufficiently captured? Many studies use rainfall or even moisture as climate indicators, but they also tend have a more agricultural focus, for example reasoning the decision to migrate or to urbanise with the changing climate that is changing agricultural yield and productivity. However, in the context of this investigation, the question is whether these indicators capture sufficiently well other quality of live-based and economic mechansims that are not mainly driven by agriculture, not least as the share of the primary sector in total GDP is and has been comparably small in Europe.
...
-   Mention number of observations, number of LAUs they cover (and the number of countries), and the time length
